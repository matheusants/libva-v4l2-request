...
void translate_matrix(
    struct v4l2_ctrl_h264_scaling_matrix *in,
...
void translate_slice(
    struct v4l2_ctrl_h264_slice_params *in,
...
static void translate_pred_weights(struct v4l2_ctrl_h264_pred_weights *dst,
                                   const struct v4l2_h264_pred_weight_table *src)
{
...
static void translate_dpb_entry(struct v4l2_h264_dpb_entry *dst,
                               const struct v4l2_h264_dpb_entry_INTERNAL *src)
{
    memset(dst, 0, sizeof(*dst));
...
static void translate_decode_params(struct v4l2_ctrl_h264_decode_params *dst,
                                    const struct v4l2_ctrl_h264_decode_params_INTERNAL *src_dec,
                                    const struct v4l2_ctrl_h264_slice_params_INTERNAL *src_slice)
{
    memset(dst, 0, sizeof(*dst));
    for (int i = 0; i < 16; i++) {
